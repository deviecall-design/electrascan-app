...
/** First word of the company name — used in the navy shell brand line. */
export function tenantBrandName(profile: CompanyProfile = getActiveCompanyProfile()): string {
  const first = profile.name.trim().split(/\s+/).filter(Boolean)[0];
  return first || "ElectraScan";
}

export function tenantInitials(profile: CompanyProfile = getActiveCompanyProfile()): string {
