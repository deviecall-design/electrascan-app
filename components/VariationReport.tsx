...
export default function VariationReport({
  ...
  // ── Export handler (generates plain-text report) ──
  const handleExport = () => {
    if (onExport) {
      onExport();
      return;
    ...
    const lines = [
      ...
      `${"═".repeat(50)}`,
      `Project: ${projectName}`,
      `Base estimate: ${baseEstNumber}`,
      `Date: ${new Date().toLocaleDateString("en-AU")}`,
      ``,
      `SUMMARY`,
      `${"─".repeat(50)}`,
      `Base total ex GST:     ${fmt(baseTotal).padStart(12)}`,
      `Net variation:         ${(delta >= 0 ? "+" : "-") + fmt(Math.abs(delta)).padStart(11)}`,
      `New total ex GST:      ${fmt(newTotal).padStart(12)}`,
      `Change:                ${pct >= 0 ? "+" : ""}${pct.toFixed(1)}%`,
      ``,
      `Items added:    ${added.length}   (+${fmt(addedDelta)})`,
      `Items removed:  ${removed.length}   (-${fmt(removedDelta)})`,
      `Items changed:  ${changed.length}`,
      ``,
      `DETAIL`,
      `${"─".repeat(50)}`,
      `${"Item".padEnd(40)} ${"Prev".padStart(5)} ${"New".padStart(5)} ${"Delta".padStart(10)}`,
      `${"─".repeat(50)}`,
      ...variationItems.map(i => {
        const d = (i.newQty - i.prevQty) * i.unitPrice;
        return `${i.description.padEnd(40)} ${String(i.prevQty).padStart(5)} ${String(i.newQty).padStart(5)} ${(d >= 0 ? "+" : "-") + fmt(Math.abs(d)).padStart(9)}`;
      }),
      `${"─".repeat(50)}`,
      ``,
      ...(risks.length > 0
        ? [
            `RISK FLAGS`,
            `${"─".repeat(50)}`,
            ...risks.map(r => `[${r.level.toUpperCase()}] ${r.title}\n  ${r.description}`),
          ]
        : []),
      ``,
      `Generated by ElectraScan · Vesh Electrical Services`,
    ].join("\n");

    const a = document.createElement("a");
    a.href = URL.createObjectURL(new Blob([lines], { type: "text/plain" }));
    a.download = `Variation_Report_${baseEstNumber}.txt`;
    a.click();
  };
  ...
  return (
    <div style={{ height: "100vh", background: C.bg, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ background: C.navy, borderBottom: `1px solid ${C.border}`, padding: "14px 18px", flexShrink: 0 }}>
        ...
        <div style={{ fontSize: 11, color: C.muted, marginBottom: 14 }}>{baseEstNumber} → New scan</div>
      ...
      <div style={{
        ...
        <button
          ...
          style={{
            flex: 2, background: C.purple, border: "none",
